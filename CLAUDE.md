...
- **Views** (`university/src/views/`, `university/src/ui/`): Godot nodes that read from state and draw it. A view owns no simulation truth — position, timers and counts live in state and the view copies them each frame. `CampusView` (`campus_view.tscn`, the main scene) is the `Node3D` root: sun, environment, ground, camera, `%Buildings`, `%BuildController` and the UI `CanvasLayer` are authored there; the script calls `GameState.update(dt)` once a frame, creates one `BuildingView` for every building already standing when it starts (a loaded game arrives with its buildings) and then creates and frees one per `Campus.BuildingAdded`/`BuildingRemoved`, and moves the selected and hovered highlights as the controller's `SelectionChanged`/`HoverChanged` report them; `Hud.setup` wires the menu's signals to the controller. It is also where the `TogglePause`, `SpeedUp` and `SpeedDown` actions become `GameState.togglePause` and `changeSpeed`, where the `QuickSave` and `QuickLoad` actions (F5, F9) become `Global.saveGame` and `loadGame` on `Global.QuickSavePath`, and where `Campus.BuildingOpened` turns that building's view solid — the side panel needs no signal for this, since it polls the selected building itself. `GameCamera` orbits a ground target at constant pitch and FOV; zoom is the distance to the target, `setTarget` clamps it to `Campus.bounds()` so every pan is limited in one place, and `_applyTransform` is the single place the transform is written, which is why `ViewMoved` is emitted from there. `GroundView` only sizes one `PlaneMesh` to `Campus.Size`; the grass and the 10 m lines are `ground.gdshader`'s, computed from world position, anti-aliased with `fwidth` and faded with distance so they never shimmer — visual only. `BuildingView` is a `BoxMesh` shaped from `building.rect().size` extruded to `Building.Height`, placed by `showRect` (the one state-angle-to-node-rotation conversion); `setHighlight` (`None`, `Selected`, `Destroy`) and `setUnderConstruction` both go through `_refresh`, the one place a building's colour is decided — the highlight picks the hue, construction drops the alpha to `ConstructionAlpha` — so a half-built box still highlights and neither setting can undo the other; `setUnderConstruction` also turns shadow casting off while the box is see-through, so a half-built building does not lay down a solid shadow. `createGhost` builds the same box translucent, with `setValid` for the green/red. `BuildController` is the only place input becomes commands, and in `campus_view.tscn` it sits **above** `Camera` in the scene tree on purpose: unhandled input is offered to nodes in reverse tree order, so the camera sees a right-button release first and swallows it when it ended a pan — what still reaches the controller is a real right *click*, which cancels the tool. UI is scene-authored: `Hud` across the top, `ActionBar` and `BuildPanel` bottom-left, `SidePanel` docked right, the lower 3/4 of the screen. Icons are PNGs in `university/assets/icons/`, supplied by the user — the fixed names (`bed`, `cal`, `coin`, `flow`, `hammer`, `menu`, `pause`, `star`, `trash`, `users`, `help`, `bell`), 64×64, a white glyph on transparent, tinted by the game, with sizes capped per button through the theme's `icon_max_width` — never SVG.
  ...
  - **`BuildPanel`** (`src/ui/build_panel.tscn`, the `build` popover, 900×430) lists `BuildingInfoDB.categories()` down the left with each one's count (`db.inCategory(category).size()`) and, for the selected category (`showCategory`), a card per type — its price, what it adds, its upkeep and its footprint (`BuildingText.adds`/`footprint`) — greyed out (`modulate.a`) while `Campus.canAfford` refuses it (`showAffordable`) and highlighted while its type is armed (`showTool`); cards are made in code because how many there are is data. Its footer names the hovered or armed type with its cost and the cash on hand, and always names when it would open; with neither hovered nor armed it reads "Choose a building to place it." Choosing a card only emits `BuildingChosen` — arming it and closing the panel is `Hud._armPlace`'s job, and closing the panel no longer cancels the tool the way the old build list did.
  - **`SidePanel`** (`src/ui/side_panel.tscn`), docked right, anchored from a quarter of the way down the screen (`anchor_top = 0.25`) to the bottom, opens on `BuildController.SelectionChanged` and closes with it (`showBuilding(null)`), so × and a left click on empty ground both close it through the existing selection-clearing they already did. It is not a popover: one of the four dimmed overlays can still open over it — the selection underneath stays live, just hidden until the overlay closes — and Esc's "clear the selection" step is what closes it, not `Hud`'s popover handling. It owns no state — everything is re-read every `_process`, so a building opening mid-view swaps its pane by itself. The header reads the type's name, "Category · what it holds" (`BuildingText.holds`) and, while the building is under construction, the accent `Pill`, "Under construction · opens Spring, Year N". The footer's Demolish button calls `Campus.destroy`, reading "Demolish · refund $2.0M" from `Campus.refundFor` while it is above 0, "Demolish" otherwise, with a Demolish (?) explaining the refund rule — the full amount while still under construction, nothing once open; the controller clears the selection when the destroyed building was selected, which is what closes the panel. Below the header, `ConstructionPane` shows while the building is going up (what opening changes — the role's capacity added, before and after; off campus before and after for housing; upkeep from then on); an open Admissions Office shows `AdmissionsPane` instead — the next fall's live projection as three stages (seats to fill, expected applicants, expected intake, the first highlighted), a bar chart of applicants per fall from `University.fallReports()` (the last `AdmissionsPane.ChartFalls`, the latest accent-coloured), and the levers — minimum entry grade, tuition, room and meal plan — as `Stepper`s calling `Policy.stepMinimum`/`stepNextTuition`/`stepNextRoom`/`stepNextMealPlan`, applying from next fall; an open academic building shows `AcademicPane` instead — three `KpiView`s (students over every open seat, open seats, this fall's entry grade), a `MeterBar` of enrolled over seats with a Seats (?) noting how many will graduate before next fall and how many seats that opens up, then every open academic building as a `BuildingRow` (a `Button` theme variation, accent-bordered on hover) reading its name, `BuildingText.holds` and its upkeep, and a jump to Admissions, hidden with none open; an open dorm shows `DormPane` — beds filled and off campus (warn past `UniversityRules.OverflowThreshold`), all dorms' beds against `Campus.beds()`, any opening at the next semester start, and this dorm's room fees (`University.roomFeesOf`) against its upkeep; an open dining hall shows `DiningPane` — meal plans, the load and who can't eat (toned by `DiningLoadLine.toneFor`), the same `DiningLoadLine` the housing dropdown uses, and all dining halls' meal plan fees against this hall's upkeep. None of the panes link out to a top-level screen any more, bar the academic pane's Admissions jump, which opens a building rather than a screen. Every pane follows the same shape: a `VBoxContainer` with `university` and `building`, filling itself in `_process` while visible; only `AcademicPane` still emits `BuildingWanted` — for its building rows and its Admissions jump — which `SidePanel` relays.
  - **`NotificationsButton`** (`src/ui/notifications_button.tscn`), between `SidePanel` and `ActionBar`/`Popovers` in `Hud`'s children so a dropdown draws over it, is the bell at the top left of the play area, under the bar: a toggled `ActionButton` (styled like Build and Demolish, so it reads as a button over the campus) whose `Badge` (a fully-rounded, bad-tinted `NotificationBadge` `PanelContainer`) shows `Count`'s digits and hides at zero, kept in step every frame by `Hud._process` calling `setCount(university.problems().size())`. Pressing it opens **`NotificationsPanel`** (`src/ui/notifications_panel.tscn`, a `Popovers` child, `BuildPanel`-styled — a full border, rounded corners and a shadow, so it reads as a floating panel rather than one hanging from the bar — under the bell) — one `MenuItemDotted` row per problem `University.problems()` reports right now, in that order, with no dates and no dismiss, or, with none, `EmptyNote`'s "Nothing needs attention." Each row carries a `StatusDot` tinted `warn` (`HousingOverflow`, `DiningCrowded`) or `bad` (`DiningUnfed`, `CreditMaxed`) through `get_theme_color`, set once when the row is built since a row's kind never changes without a rebuild; its line names the problem with its count and share, refreshed every frame since a count such as the off-campus share still moves. Rows are built in code — how many there are is live — and rebuilt only when the set of kinds changes. Clicking a row emits `ProblemWanted(kind)`, wired straight to `Hud.openProblem(kind)` (the Housing dropdown for `HousingOverflow`, the first open dining hall's pane or, with none, the Housing dropdown for `DiningCrowded`/`DiningUnfed`, the Money dropdown for `CreditMaxed`) — the place where that problem is dealt with, which already closes the popover on its way there.
  - **`SemesterPopup`** (`src/ui/semester_popup.tscn`, `Hud`'s last child, so it draws above the popovers) is the report every semester start opens: admissions (fall only — applicants, admitted with its open-seats note, entry grade, graduated), then fees collected, on/off campus, dining, reputation and satisfaction (each with its change) and what opened. `Hud.showReport`, wired to `University.SemesterStarted`, remembers `state.paused` only when the popup isn't already showing, then pauses, closes any popover and calls `SemesterPopup.showReport` — so several reports in a row show the latest while still remembering the pause from before the first of them. It has no Continue button and no (?), just its title: any left click on it — `Dim`, `Box` and `Accent` all pass the click up to the root, each with its own `STOP` mouse filter turned `PASS` — calls the public `SemesterPopup.dismiss()` (hide, then emit `Dismissed`), which hands that remembered pause back. Esc calls `dismiss()` too, checked first of all in `Hud._unhandled_input`, which while the popup is showing also marks a `TogglePause` press handled and does nothing else, so Space does nothing behind it. `SemesterStarted` fires mid-tick, before that month's bill (`University.startMonth`), so `showReport` must only pause and show — never save, load or otherwise touch state. `hud popup` (`showLatestReport`) shows the latest `University.reports` entry, for the console.
  ...
  - **`TopBar`** holds:
    - ☰ (Game menu) and the crest (University menu). The crest is a fixed 440 px wide, which the centred slots leave room for even with much bigger numbers: its name shows at the scene's 22 px and `TopBar._fittingNameSize` shrinks it a pixel at a time until it fits, down to `MinNameFontSize`, past which it is clipped (`clip_text`) — never an ellipsis
    - the six `SlotView`s, centred on the bar whatever sits either side of them: they sit in `Center`, a `CenterContainer` laid over `Row` with `mouse_filter = 2` so the ☰, crest and transport buttons under it still take clicks, with separators between slots only. The bar's right padding is `Row`'s `EndMargin` spacer rather than the `TopBar` stylebox's margin, which would pull the centre off the screen's. One line each: an icon, the number (`Heading`) and a note of numbers only (`Caption`), both 16 px on one baseline — no caption and no words, so a slot is named by its `tooltip_text` instead (the last argument of `SlotView.display`). Students / seats, beds + off campus (warn past the threshold), cash with any debt as a red `−$2.0M`, monthly expenses, next fees (its tooltip names the semester and the weeks until it) and reputation → its target
    - the clock: `GameCalendar.periodLabel` and `clockLine` on one line, over a `MeterBar` of the period's progress
  ...
  - **The four dropdowns off the top bar** (`StudentsDropdown`, `HousingDropdown`, `MoneyDropdown`, `ReputationDropdown`) are centred overlays covering 80% of the play area over a dimmed campus, rather than compact panels: the root is a plain `Control`, full rect with `offset_top = 48` and `visible = false`, its default `STOP` mouse filter keeping every click off the world underneath. Its first child, `Dim`, is a full-rect `Panel` with the `PopupDim` theme variation (black at 0.45, the one the semester popup dims with) and its own `STOP` filter, carrying the shared `ClickAway` script (`src/ui/click_away.gd`, one copy instead of four identical handlers); its `Clicked` signal, connected in each script's `_ready` to emit `CloseRequested`, is what a click outside the panel closes it with. Its second child, `Frame`, is a `PanelContainer` with the `Overlay` theme variation (`panel_bg`, no border) anchored to the centre 80% of the play area (`anchor_left`/`anchor_top` 0.1, `anchor_right`/`anchor_bottom` 0.9) on top of `Dim`, so a click inside it stops at `Frame`'s own `STOP` filter and never reaches the dim underneath. `Frame` holds the `ScrollContainer` (vertical only, `horizontal_scroll_mode = 0`) that fills it and centres a `Content` `VBoxContainer` capped at 1360 px wide, so a table taller than the frame scrolls instead of running off the bottom. Its first child is a header row — the title, any `HelpIcon`, nothing after it now that there is no × — above the KPI row and a body laid out for the full width as `size_flags_horizontal = 3` columns (`separation = 48`). Closing also happens through the slot or menu entry that opened it (toggled) and through Esc. While one of the four is open, `Hud.openPopover`/`closePopover` set `GameCamera.keysEnabled` and `BuildController.keysEnabled` to false, so the camera's pan/turn/zoom keys and the ghost's rotate keys stop polling underneath it, and `Hud._unhandled_input` marks every other key event or action handled before it reaches `BuildController`, `GameCamera` or `CampusView` — only `TogglePause`, `SpeedUp` and `SpeedDown` fall through, so Space and +/− still work; the wheel needs no gate, since the dim or the panel already takes it as a GUI event. The game keeps running behind an open overlay — only the semester popup pauses it.
  ...
  - **`ReputationDropdown`** (the Reputation slot, and the university menu's Reputation entry) shows reputation, its `reputationTarget()` and where `UniversityRules.nextReputation` would take it by next fall, then a "Target" column (?) — the latest intake's grade and the year's satisfaction, its two halves — beside `satisfactionNow()`'s three penalties (housing, crowding, unfed). It carries no jump of its own; the Students and Housing dropdowns keep theirs.
  ...
  `UiTone` (a tone is a theme variation suffix — `Good`/`Warn`/`Bad`/`Dim` on a base type such as `Value`), `KpiView`, `InfoRow`, `SlotView`, `MeterBar` and `HelpIcon` (`src/ui/`) are the HUD's shared building blocks, coloured through `ui_theme.tres`'s `Palette`. `HelpIcon` is the (?) beside a heading or value that needs explaining: a `TextureRect` showing `help.png`, sized 20×20 with `mouse_filter = 0` so it takes the hover, whose `tooltip_text` — authored in the scene when the text is fixed, or set by the owning view (in `_ready` or `_process`, reusing the same format const the old note used) when a number feeds it — shows through `_make_custom_tooltip` as a `Body` `Label` wrapped at `HelpIcon.WrapWidth`, styled by `ui_theme.tres`'s `TooltipPanel`/`TooltipLabel`. The rule it follows: a paragraph that explains how something works moves out of the view and behind a `HelpIcon`; a label that only states a number or a fact (a KPI's caption, "counts as 78", "No fall admission yet.") stays a plain label. Every Label in the HUD carries exactly one of five theme type variations: `Title` (26 px, the Cormorant `SerifBold` `FontVariation`, no tones — the crest name, at 22 px or less through the HUD's one font-size override, the shield letter, the popups' headings, the four overlay titles and the side panel's building name), `Heading` (16 px, Barlow Semi Condensed SemiBold — section titles and table headers, set uppercase, and the top bar's slot numbers, the same size as their notes), `Value` (24 px, Barlow Semi Condensed SemiBold — the KPIs' headline numbers), `Body` (18 px, Barlow Regular, also the theme's `default_font`/`default_font_size` — row keys and values, table cells, the base `Button`) and `Caption` (16 px, Barlow Semi Condensed Medium, muted — sub-labels and notes); `Heading`, `Value`, `Body` and `Caption` each carry the four tones as a `Good`/`Warn`/`Bad`/`Dim` suffix through `UiTone.variation`, `Title` has none. `Pill`/`PillGood`/`PillWarn`/`PillBad`/`PillDim` keep their own coloured background box but take `Caption`'s font and size. Buttons map onto the same five: menu items, building rows, category buttons, cards, `GhostButton` and the base `Button` are `Body`; the speed buttons, the action buttons and jump links are `Heading`; the ×/−/+ glyph buttons (`CloseButton`, `StepButton`) are `Value`. `Palette` holds `panel_bg`/`panel_raised`/`panel_raised2` (a panel's background, a surface raised on it such as a hover, a card or a tooltip, and what must stand out on that, such as a meter's track), `line`/`line2`, `text`/`muted`/`dim`, `good`/`warn`/`bad`/`maroon` and the blue accent — `accent` (#4a90e2) and `accent2` (#8cc2ff) for highlights, borders and hovers, `onAccent` for text sat on an accent-filled control — everywhere gold used to be; `warn` (amber) is unchanged, so a problem still reads as a warning rather than a highlight. The top bar is 48 px tall; every dropdown and menu sit at `offset_top = 48` to clear it — `NotificationsPanel` is the exception, sitting at 124 under the bell, and the four overlays' `Frame` is anchored inside their `Dim` rather than carrying its own `offset_top = 48` — `NotificationsButton` sits at 60 to clear the bar plus its own margin, and the side panel sits at `anchor_top = 0.25` instead, clearing the bar by a wide margin since it only covers the lower 3/4 of the screen. `MoneyFormat.short` is the one place a balance is abbreviated — `$10.0M`, `$950K`, `$0` — used by the top bar and by the build panel's cards alike; `full` groups whole dollars (`$9,606`) and `signed` reads a balance with its direction (`+$16.6M`, `−$1.9M`). `NumberFormat.count` groups a whole number (`1,080`) and `percent` floors a fraction to a whole share (`33%`).
...
colours (`panel_bg`, `accent`, `warn`, `shadow`, ...), so Godot's Inspector
...
Monthly expenses or Fees the Money dropdown (what `F3` opens too), with all
three reading pressed together while Money is open. The dropdowns carry
jumps onward to what they don't already show — Students to Housing and
Reputation, and Housing's "Room & meal plan prices →"
to the Admissions pane, disabled with no office open; Reputation carries none
of its own. Each of the four is a
centred panel over a dimmed campus, covering 80% of the play area under the
top bar; a click anywhere on the dim outside it closes it, so does clicking
the slot or crest entry that opened it, and so does Esc — there is no ×.
While one is open, only Esc, Space and +/− still do anything: every other key
and console action is swallowed, so a rotate key can't turn a ghost or WASD
pan the camera underneath it, and the game keeps running behind it. With no tool armed, LMB
...
total, the academic pane alone still jumping onward, to Admissions. Its own Demolish button destroys the selected
...
`Finances.LoanStep` steps. Top-left, under the bar, the bell shows a red count
of whatever is wrong now; pressing it opens a panel listing those problems,
with no dates and no dismiss, and a click on one opens its place. Every semester
start opens the report popup and pauses the game; a click anywhere on it, or
Esc, resumes whatever was running before it — there is no Continue button and
no (?). Space, `B`, `X`, `Tab`, `F2` and `F3` do nothing while it is up.
