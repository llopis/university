...
- **Save/load.** A save is one JSON dictionary. Every state class writes itself with a pure `toDict()` and rebuilds with `static fromDict(data, buildingDB)`:
  ...
  - `University`, `Campus`, and `Building`, which is saved by type id — unlike the others, its `fromDict(data, buildingInfo)` takes the type already looked up rather than the whole `buildingDB`. A type the building data no longer has is reported (`Campus.UnknownTypeError`) and left out.
...
code ever sees $M. A renamed or missing column reads as free, not as an error. Its `buildTime` column is still
