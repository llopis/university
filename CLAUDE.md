...
- `hud <name>` — open a HUD popover by name (`gamemenu`, `unimenu`, `students`, `housing`, `money`, `reputation`) or a side-panel pane by name (`construction`, `admissions`, selecting its first building and moving the camera there); `popup` shows the latest semester report; `none` closes whichever popover is open.
