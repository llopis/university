...
The `Buildings` tab authors `cost` in thousands of dollars:
`BuildingInfo.DollarsPerK` turns it into whole dollars in the constructor and
nowhere after, so no other code ever sees $K. Its `buildTime` column is still
parsed and deliberately unread — construction ends at the next semester start,
not after a duration.
...
pressed, and the date and the balance sit to their right. The speed buttons
...
balance, then `Destroy`. Choosing a type arms it — a translucent ghost follows
